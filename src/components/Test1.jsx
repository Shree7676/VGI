...
function Test1() {
  ...
  const handleTripClick = (tripId, stop_id) => {
    const stopData = stopsData.current.find(stop => stop.stop_id === stop_id);

    setLocation(stopData)
  ...
  return (
    <MapContainer center={defaultPosition} zoom={10} style={{ height: '100vh', width: '100%' }}>
      ...
      {selectedTripId && location && <MovingBus tripId={selectedTripId} location={location} stopsData={stopsData} stopTimesData={stopTimesData} />}
    </MapContainer>
  );
}
